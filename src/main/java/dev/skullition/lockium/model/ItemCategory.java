 ...
 *
 * @see dev.skullition.lockium.model.GrowtopiaObject.CategoryInfo
 * @since 0.1.0
 */
public enum ItemCategory {
  ...
  // 121 unused
  TOGGLEABLE_MULTI_FRAMED_ANIMATED_FOREGROUND_BLOCKS(
      122, "Gravity Well", "Toggleable Multi-Framed Animated Foreground Blocks"),
   ...
   *
   * @param id the numeric ID used by the API
   * @param icon representative item name used as an icon fallback; may be {@code null}
   * @param itemName pluralized display name for UI grouping; may be {@code null}
   */
  ...
  /**
   * Resolves a category by its numeric ID.
   *
   * @param id the API category ID
   * @return the matching constant, or {@link #UNKNOWN} if the ID is not defined (including gaps
   *     like 30, 48, 121)
   */
  ...
  /**
   * Returns the human-readable text for this category.
   *
